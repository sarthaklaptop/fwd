...
function FancyButton({
  ...
  const glowStyles = {
    ...
    emerald: {
      radial:
        'bg-[image:radial-gradient(75%_100%_at_50%_0%,rgba(52,211,153,0.6)_0%,rgba(52,211,153,0)_75%)]',
  ...
  return (
    ...
    >
      <span className="absolute inset-0 overflow-hidden rounded-full">
        <span
          className={`absolute inset-0 rounded-full ${glowStyles[glowColor].radial} opacity-0 transition-opacity duration-500 group-hover:opacity-100`}
        />
      </span>
      ...
      <span
        className={`absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r ${glowStyles[glowColor].line} transition-opacity duration-500 group-hover:opacity-40`}
      />
    </a>
  );
}
